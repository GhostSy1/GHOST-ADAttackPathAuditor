...
TOOL = "GHOST-ADAttackPathAuditor"
VERSION = "3.0-PRO"
PROFILE = "Active Directory attack path and misconfiguration review"
RULES = [
    ("AD-DELEGATION", r"unconstrained delegation|constrained delegation|rbcd|trustedfordelegation"),
    ("AD-ACCOUNT", r"adminCount=1|domain admins|enterprise admins|krbtgt|as-rep-roastable|kerberoastable"),
    ("AD-TRUST", r"external trust|forest trust|sid-history|selective authentication"),
    ("AD-POLICY", r"password policy|sysvol|gpp|group policy|acl|genericall|writedac")
]
...
def render_banner() -> None:
 ...
 | |__| | | | | |__| | |__| || |_
  \\_____|_| |_|\\____/ \\____/_____|
      GHOST-ADAttackPathAuditor v3.0-PRO (Zero-Guessing Engine)
\"\"\"
    print(banner)
    ...
    for line_no, line in enumerate(text.splitlines(), 1):
        for rule_id, pattern in RULES:
            if re.search(pattern, line, re.I):
                findings.append({
                    "rule_id": rule_id,
                    "severity": "high" if "DELEGATION" in rule_id or "ACCOUNT" in rule_id else "medium",
                    "confidence": "high",
                    "title": f"AD Attack Path Indicator: {{rule_id}}",
                    "description": f"Matched pattern {{rule_id}} in Active Directory export artifact.",
                    "evidence": evidence(line),
                    "source": "operator-input",
                    "location": f"{{path}}:{{line_no}}",
                    "remediation": "Review Active Directory ACEs, delegation settings, and credential hygiene."
                })
    return findings
...
def run(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--input", type=Path, help="Path to AD export file or directory")
    ...
    if not target:
        if sys.stdin.isatty():
            try:
                raw = input("Enter Active Directory export path: ").strip()
                if raw:
                    target = Path(raw)
    ...
    print(f"[*] Analyzing Active Directory target: {{target.resolve()}} (Strict Zero-Simulation Mode)...")
